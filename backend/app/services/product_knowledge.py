...
def _reference_image_block(image_count: int = 1) -> str:
    ...
    return (
        ...
        "These images are the ground-truth product / logo / packaging. "
        "Downstream image generation will also receive the same pixels — your prompt "
        "must describe them accurately so the scene brief stays consistent.\n\n"
        "YOU MUST:\n"
        ...
        "- PRODUCT / LOGO FIDELITY (critical):\n"
        "  • Treat packaging, logo artwork, label layout, brand mark, printed text, "
        "colors, shape, and proportions as FIXED — do not redesign or restyle them.\n"
        "  • Describe the product appearance precisely (logo placement, label wording "
        "you can read, color hex-ish cues, bottle/box geometry) so generation stays "
        "faithful.\n"
        "  • Scene, lighting, camera, and props may change per the user's request; "
        "the product itself must remain the same object from the attachment(s).\n"
        ...
        "- Invent a different logo, label layout, packaging design, or brand colors.\n"
...
def _intent_mode_block(intent: CreationIntent) -> str:
    ...
    if intent == CreationIntent.CREATE_IMAGE:
        return (
            ...
            "plus any attached reference images to the image API (product-reference mode).\n\n"
            "YOU MUST:\n"
            ...
            "- If references show a product/logo: describe keeping that exact packaging, "
            "logo, label text, colors, shape, and proportions while applying the user's "
            "scene / lighting / composition request.\n\n"
            "YOU MUST NEVER:\n"
            ...
            "- Redesign the product or invent alternate branding.\n"
