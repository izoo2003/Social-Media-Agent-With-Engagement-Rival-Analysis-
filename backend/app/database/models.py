...
class CampaignStatus(str, PyEnum):
    """Lifecycle of a marketing campaign plan."""
...
class CampaignAssetType(str, PyEnum):
    """Creative asset types a campaign plan can schedule."""
...
class Campaign(Base):
    """
    Multi-product/category social campaign plan.

    Generated by Gemini into CampaignItems, then optionally committed into
    Content + CalendarEvent rows for auto-publishing (after media is attached).
    """
    ...
    products = Column(JSON, nullable=True)  # [{product?, category}, ...]
    status = Column(String(50), default=CampaignStatus.DRAFT.value)
    plan_summary = Column(JSON, nullable=True)  # counts by asset_type
    ...
    items = relationship(
        ...
        order_by="CampaignItem.sort_order",
    )


class CampaignItem(Base):
    """A single planned post slot within a campaign timeline."""
    ...
    scheduled_at_pkt = Column(String(64), nullable=True)  # ISO display string for UI
    platforms = Column(JSON, nullable=True)
    asset_type = Column(String(50), nullable=False, default=CampaignAssetType.POST_IMAGE.value)
    ...
    calendar_event_id = Column(Integer, ForeignKey("calendar_event.id"), nullable=True)
